...
foreach( $skills as $att => $value ){
    if( $attack_skills[$att] == null ) {
        if($dice[$_POST[$att]-1]){
           $pdf->setX( 0.10);
           $pdf->Cell( 0.2, 0.145, substr( $dice[$_POST[$att]-1], 1), 0, 0,"L", false );
           #$pdf->Cell( 0.2, 0.145, "yyy", 0, 0,"L", false );
           $pdf->Cell( 0.90, 0.145, ucwords( $att ), 0, 2,"R", false );
        }
    }else{
        if($dice[$_POST[$att]/3]){
           $pdf->setX( 0.10);
           $pdf->Cell( 0.2, 0.145, substr( $dice[$_POST[$att]/3], 1), 0, 0,"L", false );
           #$pdf->Cell( 0.2, 0.145, "xxx", 0, 0,"L", false );
           $pdf->Cell( 0.90, 0.145, ucwords( $att ), 0, 2,"R", false );
        }
    }
}
